...
describe('XML Namespace Parse', () => {
	it("supports testlitecns", () => {
		...
		strictEqual(el.namespaceURI, 'http://www.xidea.org/lite/core')
		strictEqual(doc.toString(), '<html xmlns="http://www.w3.org/1999/xhtml"><body><c:var name="a" value="${1}" xmlns:c="http://www.xidea.org/lite/core"></c:var></body></html>')
	})
	...
	it("test", () => {
		...
		for (var _i = 0, els_1 = els; _i < els_1.length; _i++) {
		    ...
		    var te = doc.createElementNS(n1, "test");
		    te.setAttributeNS(n1, "bar", "valx");
		    strictEqual(String(te),'<test xmlns="'+n1+'" bar="valx"/>', `1. i ${_i}, ${el}`);
		    el.appendChild(te);
		    var tx = doc.createElementNS(n2, "test");
		    tx.setAttributeNS(n2, "bar", "valx");
		    strictEqual(String(tx),'<test xmlns="'+n2+'" bar="valx"/>', `2. i ${_i}, ${el}`);
		    el.appendChild(tx);
		}
		var sr = String(doc);
		strictEqual(sr, '<html test="a" xmlns="http://www.w3.org/1999/xhtml" xmlns:rmf="http://www.frankston.com/public"><rmf:foo hello="asdfa"><test xmlns="http://www.frankston.com/public" bar="valx"></test><test xmlns="http://rmf.vc/n2" bar="valx"></test></rmf:foo></html>');
