...
describe('XML attrs', () => {
    ...
    it("can properly set ns attribute", () => {
    	...
    	strictEqual(child.attributes.length, 3, 'after adding 3', child);
    	child.setAttribute('a',1);
    	child.setAttributeNS('b','b:b','2');
    	strictEqual(child.attributes.length, 4, 'after adding 4 and one with namespace');
    	try {
    		var c = root.ownerDocument.createElement('c');
    		c.setAttributeNodeNS(root.attributes.item(0));
    	} catch (e) {
    		strictEqual(e.code, 10, 'wrong error code');
    		return;
    	}
    	fail('expected error but none was thrown');
    })
    ...
    it("can properly override attribute", () => {
    	...
    	strictEqual(root.attributes.length, 4);
//not standart
...
//    	assert(root.attributes.length, 0);
    ...
    it("properly supports attribute namespace", () => {
    	...
    	strictEqual(root.getAttributeNS("a", "b"), "e");
    })
